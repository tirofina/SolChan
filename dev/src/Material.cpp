...
void Material::replace_material(string name, int val) {
	for (int i = 0; i < num; i++) {
		if (!name.compare(material_name[i])) {
			material_amount[i] = val;
			break;
		}
	}
}
...
bool Material::check_material(string name) {
	for (int i = 0; i < num; i++) {
		if (!name.compare(material_name[i]))
			return true;
	}
...
int Material::name_price(string name) {
	for (int i = 0; i < num; i++) {
		if (!name.compare(material_name[i]))
			return material_price[i];
	}
	return -1;
}
